#!/usr/bin/env python3
"""
🤖 RoArm MCP - Isaac Sim 로봇 제어 인터페이스 구현 (수정판)
...
def test_robot_control_interface():
    ...
    try:
        ...
        # Isaac Sim 5.0 호환 모듈 import (SimulationApp 초기화 후)
        import omni.usd
        import numpy as np
        import time
        ...
        # 로봇 모델 로딩
        ur10_robot, franka_robot = load_robot_models(stage, world, add_reference_to_stage, UsdGeom, Gf)
        
        # 로봇 제어 인터페이스 생성
        ur10_controller, franka_controller = create_robot_controllers(stage, world, ur10_robot, franka_robot, SingleArticulation, np)
...
def load_robot_models(stage, world, add_reference_to_stage, UsdGeom, Gf):
...
def create_robot_controllers(stage, world, ur10_robot, franka_robot, SingleArticulation, np):
    """로봇 제어 인터페이스 생성"""
    print("\n🎮 로봇 제어 인터페이스 생성...")
...
class RobotController:
    ...
    def get_joint_positions(self):
        ...
        try:
            ...
            return self.np.zeros(self.num_dof) if self.num_dof > 0 else None
